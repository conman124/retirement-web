...
export function graph(retirement: typeof import("@conman124/retirement"), svgEl: SVGElement, showSimCount: number) {
    const { AccountContributionSettings, AccountContributionSettingsVec, AccountContributionSource, AccountContributionTaxability, AccountSettings, AssetAllocation, FicaJS, JobSettings, PersonSettings, RaiseSettings, RatesSourceHolder, Simulation, TaxSettings } = retirement;
    ...
    let taxSettings = new TaxSettings(new Float64Array([0, 10275, 41775, 89075, 170050, 215950, 539900]), new Float64Array([0.1, 0.12, 0.22, 0.24, 0.32, 0.35, 0.37]), true, 12950, true);
    ...
    const time = d3.scaleUtc().domain([dates[0], dates[dates.length-1]]).range([margin.left, width - margin.right]);
    const money = d3.scaleLinear().domain([0, maxBalance]).range([height - margin.bottom, margin.top]);    
    const line = d3.line()
        .x(([,d]) => time(d))
        .y(([d]) => money(d));

    for(let i = 0; i < showSimCount; ++i) {
        const data_joined = Array.prototype.map.call(simulation.get_account_balance_for_run(i, 0), (balance, index) => [balance, dates[index]]);
        const successful = simulation.assets_adequate_periods_for_run(i) >= simulation.lifespan_for_run(i).periods();

        svg.append('path')
            .attr('fill', 'none')
            .attr('stroke', successful ? "rgba(0, 255, 0, 0.4)" : "rgb(255, 0, 0, 0.4)")
            .attr("stroke-width", "1")
            ...
            .attrTween("stroke", () => d3.interpolate("rgba(128, 128, 128, 0.4)", successful ? "rgba(0, 255, 0, 0.4)" : "rgb(255, 0, 0, 0.4)"));
    }

    const xAxis = g => { g
        .attr("transform", `translate(0,${height - margin.bottom})`)
        .call(d3.axisBottom(time).ticks(20))
    }
    const yAxis = g => { g
        .attr("transform", `translate(${margin.left},0)`)
        .call(d3.axisLeft(money).ticks(5, "$,.0f"))
    }
    svg.append("g")
    .call(xAxis);
